#!/bin/bash
#SBATCH --job-name=build-pypsa-earth-gurobi10
...
ENV_PREFIX=/data/engs-df-green-ammonia/engs2523/envs/pypsa-earth-env-gurobi10
LOGDIR=/data/engs-df-green-ammonia/engs2523/envs/logs
mkdir -p "$LOGDIR" "$(dirname "$ENV_PREFIX")"

# Recreate env from the ARC-specific spec
rm -rf "$ENV_PREFIX"
conda env create -y -p "$ENV_PREFIX" -f "$WORKDIR/envs/environment-arc-gurobi10.yaml"

# Snapshot package list
conda list -p "$ENV_PREFIX" > "$LOGDIR/pypsa-earth-env-gurobi10-packages.txt"
